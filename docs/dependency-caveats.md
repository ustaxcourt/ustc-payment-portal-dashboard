...
### next 15.5.22 → 16.3.4 — deferred (2026-07-27, re-confirmed 2026-09-08)
**AWS has no Next 16 support as of Sept 8th, 2026**

- **Current:** `15.5.22` (pinned exactly, not a range). **Available latest:** `16.3.4`.
...
- **Reason it can't be fixed now:** These advisories cover `postcss <= 8.5.22`.
  Our direct dependency tree is already clear — `@tailwindcss/postcss@4.3.3`
  resolves `postcss@8.5.28`, which is patched. The flagged copy is
  `postcss@8.4.31`, vendored inside `next@15.5.22` at
  `node_modules/next/node_modules/postcss` (still present in `next@15.5.25`, the
  latest 15.x). We do not control that pin, and `npm audit fix --force` "resolves"
  it by installing `next@16.3.4` — the deferred major (see
...
- **Revisit:** Clears with the Next 16 upgrade (`next@16.3.4` vendors
  `postcss@8.5.23`). Re-run `npm audit` on each dependency-update cycle and drop
  this entry once the nested copy is patched.
...
- **Reason it can't be fixed now:** `sharp < 0.35.0` inherits libvips
  ...
  the only fix `npm audit` offers is the deferred `next@16.3.4` upgrade.
- **Mitigation:** The vulnerabilities are in libvips image decoding, reachable
  only by passing untrusted image bytes through `sharp`. This app has no image
  pipeline: no `next/image`, no `public/` directory, and the one raster asset
  (`src/app/icon.png`, a first-party app icon) is not run through `sharp`. No code
  path hands an attacker-supplied image to `sharp`. **Re-evaluate before adding
  any user-supplied image upload, remote image optimization, or `next/image` use.**
- **Revisit:** When Next.js bumps its bundled `sharp` to `>= 0.35.0` (or an Upgrade to Next 16), or sooner
